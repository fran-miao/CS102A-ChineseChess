...
public class CannonChessComponent extends ChessComponent {
    ...
    public boolean canMoveTo(ChessComponent[][] chessboard, ChessboardPoint destination) {
        ...
        if (source.getX() == destination.getX()) {
            ...
            if((breaks==1&&chessboard[destination.getX()][destination.getY()] instanceof EmptySlotComponent)||breaks>1
                    ||(breaks==0&&!(chessboard[destination.getX()][destination.getY()] instanceof EmptySlotComponent)))
                return false;
...
if(isPossishow())g.setColor(STEP_COLOR);
        ...
        if (getChessColor().equals(ChessColor.RED))
            g.drawString("炮", 12, 25); // FIXME: Use library to find the correct offset.
        if (getChessColor().equals(ChessColor.BLACK))
            g.drawString("砲", 12, 25);
